...
public class SetOperationTest extends AbstractRedisTest {
    ...
    public void testSintercard() {
        String[] r = Server.random(10);
        assertEquals(0, jedis.sintercard(r[0], r[1]));
        for (int i = 0; i < 5; i++) {
            jedis.sadd(r[0], r[i]);
        }
        assertEquals(0, jedis.sintercard(r[0], r[1]));
        ...
        assertEquals(3, jedis.sintercard(r[0], r[1]));
        for (int i = 0; i < 5; i++) {
            jedis.sadd(r[2], r[i + 4]);
        }
        assertEquals(1, jedis.sintercard(r[0], r[1], r[2]));
        // with LIMIT: sintercard(limit, key1, key2...)
        long withLimit = jedis.sintercard(1, r[0], r[1]);
        assertEquals(1, withLimit);
    }
    ...
    public void testSmismember() {
        String[] r = Server.random(5);
        List<Boolean> result = jedis.smismember(r[0], r[1], r[2]);
        assertTrue(result.isEmpty() || result.stream().noneMatch(x -> x));
        jedis.sadd(r[0], r[1], r[3]);
        result = jedis.smismember(r[0], r[1], r[2], r[3]);
        assertEquals(3, result.size());  // key + 3 members -> 3 results
        assertTrue(result.get(0));   // r[1] in set
        assertFalse(result.get(1));  // r[2] not in set
        assertTrue(result.get(2));   // r[3] in set
    }
