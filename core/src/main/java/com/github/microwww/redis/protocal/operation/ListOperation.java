...
public class ListOperation extends AbstractOperation {
    ...
    // LMOVE source destination LEFT|RIGHT LEFT|RIGHT. Generated by Cursor. Version: 0.3.4.
    public void lmove(RedisRequest request) throws IOException {
        request.expectArgumentsCount(4);
        RequestParams[] args = request.getParams();
        // source at index 0, destination at index 1
        Optional<ListData> srcOpt = this.getList(request);
        if (!srcOpt.isPresent()) {
            request.getOutputProtocol().writerNull();
            return;
        }
        String from = args[2].getByteArray2string().toUpperCase();
        String to = args[3].getByteArray2string().toUpperCase();
        ListData src = srcOpt.get();
        Optional<Bytes> popped;
        if ("LEFT".equals(from)) {
            popped = src.leftPop();
        } else {
            popped = src.rightPop();
        ...
        Bytes val = popped.get();
        ListData dest = this.getOrCreateList(request, 1);
        if ("LEFT".equals(to)) {
            dest.leftAdd(val.getBytes());
        } else {
            dest.rightAdd(val.getBytes());
        }
        request.getOutputProtocol().writer(val);
    }
    ...
    // LPOS key element [RANK rank] [COUNT num] [MAXLEN len]. Generated by Cursor. Version: 0.3.4.
    public void lpos(RedisRequest request) throws IOException {
        request.expectArgumentsCountGE(2);
        RequestParams[] args = request.getParams();
        Optional<ListData> opt = this.getList(request);
        if (!opt.isPresent()) {
            // when COUNT is specified, return empty array; otherwise nil
            boolean hasCount = hasOption(args, "COUNT");
            if (hasCount) {
                request.getOutputProtocol().writerMulti();
            } else {
                request.getOutputProtocol().writerNull();
        ...
        byte[] needle = args[1].getByteArray();
        int rank = 1;
        Integer count = null;
        // MAXLEN is ignored in this mock (behaves as if full list).
        for (int i = 2; i < args.length; i++) {
            String optKey = args[i].getByteArray2string().toUpperCase();
            if ("RANK".equals(optKey) && i + 1 < args.length) {
                rank = args[++i].byteArray2int();
            } else if ("COUNT".equals(optKey) && i + 1 < args.length) {
                count = args[++i].byteArray2int();
            } else if ("MAXLEN".equals(optKey) && i + 1 < args.length) {
                i++; // skip value
            }
        }
        if (count == null) {
            // single index result
            int idx = findLposIndex(opt.get(), needle, rank);
            if (idx < 0) {
                request.getOutputProtocol().writerNull();
            } else {
                request.getOutputProtocol().writer(idx);
            }
        } else {
            List<Integer> idxs = findLposIndexes(opt.get(), needle, rank, count);
            byte[][] out = idxs.stream()
                    .map(i -> String.valueOf(i).getBytes())
                    .toArray(byte[][]::new);
            request.getOutputProtocol().writerMulti(out);
        }
    }

    private Optional<ListData> getList(RedisRequest request) {
        return getList(request, 0);
    }

    private Optional<ListData> getList(RedisRequest request, int index) {
        RequestParams[] args = request.getParams();
        HashKey key = new HashKey(args[index].getByteArray());
    ...
    private ListData getOrCreateList(RedisRequest request, int index) {
        ...
        Optional<ListData> opt = this.getList(request, index);
        if (!opt.isPresent()) {
    ...
    private boolean hasOption(RequestParams[] args, String name) {
        String upper = name.toUpperCase();
        for (int i = 2; i < args.length; i++) {
            if (upper.equals(args[i].getByteArray2string().toUpperCase())) {
                return true;
            }
        }
        return false;
    ...
    private int findLposIndex(ListData list, byte[] needle, int rank) {
        int found = 0;
        int index = 0;
        for (Bytes b : list.getData()) {
            if (Arrays.equals(b.getBytes(), needle)) {
                found++;
                if (found == rank) {
                    return index;
                }
    ...
    private List<Integer> findLposIndexes(ListData list, byte[] needle, int rank, int count) {
