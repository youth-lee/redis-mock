...
public class HashOperation extends AbstractOperation {
    ...
    // HSTRLEN. Generated by Cursor. Version: 0.3.4.
    public void hstrlen(RedisRequest request) throws IOException {
        request.expectArgumentsCount(2);
        RequestParams[] args = request.getParams();
        HashKey field = args[1].byteArray2hashKey();
        Optional<Map<HashKey, Bytes>> map = this.getHashMap(request);
        int len = map.map(m -> {
            Bytes v = m.get(field);
            return v == null ? 0 : v.getBytes().length;
        }).orElse(0);
        request.getOutputProtocol().writer(len);
    }

    // HRANDFIELD. Generated by Cursor. Version: 0.3.4.
    public void hrandfield(RedisRequest request) throws IOException {
        request.expectArgumentsCountGE(1);
        RequestParams[] args = request.getParams();
        Optional<Map<HashKey, Bytes>> mapOpt = this.getHashMap(request);
        Map<HashKey, Bytes> map = mapOpt.orElse(Collections.emptyMap());
        int argc = args.length;
        if (argc == 1) {
            // Single field or null
            if (map.isEmpty()) {
                request.getOutputProtocol().writerNull();
                return;
            }
            List<HashKey> fields = new ArrayList<>(map.keySet());
            HashKey field = fields.get(new Random().nextInt(fields.size()));
            request.getOutputProtocol().writer(field.getBytes());
            return;
        }
        ...
        if (argc > 2) {
            String opt = args[2].getByteArray2string().toUpperCase();
            if ("WITHVALUES".equals(opt)) {
                withValues = true;
            }
        }
        List<HashKey> fields = new ArrayList<>(map.keySet());
        ...
        if (count >= 0) {
            ...
            int limit = Math.min(count, fields.size());
            for (int i = 0; i < limit; i++) {
                HashKey f = fields.get(i);
                out.add(f.getBytes());
                if (withValues) {
                    out.add(map.get(f).getBytes());
                }
            }
        } else {
            int need = -count;
            for (int i = 0; i < need; i++) {
                ...
                HashKey f = fields.get(rnd.nextInt(fields.size()));
                out.add(f.getBytes());
                if (withValues) {
                    out.add(map.get(f).getBytes());
                }
        ...
        request.getOutputProtocol().writerMulti(out.toArray(new byte[out.size()][]));
    }
