...
public class SetOperation extends AbstractOperation {
    ...
    // SINTERCARD. Generated by Cursor. Version: 0.3.4.
    public void sintercard(RedisRequest request) throws IOException {
        request.expectArgumentsCountGE(2);
        RequestParams[] args = request.getParams();
        int numkeys = args[0].byteArray2int();
        if (numkeys <= 0 || 1 + numkeys > args.length) {
            throw new IllegalArgumentException("numkeys must be positive and match number of keys");
        }
        int limit = 0;
        int keyEnd = 1 + numkeys;
        if (keyEnd < args.length) {
            String opt = new String(args[keyEnd].getByteArray()).toUpperCase();
            if ("LIMIT".equals(opt) && keyEnd + 1 < args.length) {
                limit = args[keyEnd + 1].byteArray2int();
            }
        }
        HashKey[] keys = Arrays.stream(args, 1, keyEnd).map(e -> e.byteArray2hashKey())
                .toArray(HashKey[]::new);
        Set<Bytes> inter = new SetData().interStore(request.getDatabase(), keys);
    ...
    // SMISMEMBER. Generated by Cursor. Version: 0.3.4.
    public void smismember(RedisRequest request) throws IOException {
        request.expectArgumentsCountGE(2);
        RequestParams[] args = request.getParams();
        HashKey hk = args[0].byteArray2hashKey();
        Optional<SetData> data = request.getDatabase().get(hk, SetData.class);
        Set<Bytes> set = data.map(SetData::getData).orElse(Collections.emptySet());

        Long[] result = new Long[args.length - 1];
        for (int i = 1; i < args.length; i++) {
            result[i - 1] = set.contains(args[i].toBytes()) ? 1L : 0L;
        }
        request.getOutputProtocol().writerComplex(result);
    }
